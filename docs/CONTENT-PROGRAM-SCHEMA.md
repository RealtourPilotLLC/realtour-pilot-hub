...
`ProgramAutomation.key` values and what each gates: `reminders` (§24) · `transcript_jobs` (§20) · `ai_runs` (§13, master switch for ProgramAiRun execution) · `publishing` (§12) · `script_share_email` (§22 Approve & share email) · `script_auto_share` (6.5, unified handoff Sep 25 2026 — a clean, sweep-drafted current version is approved and released to the portal WITHOUT Jordan's individual approval after `holdMinutes` (120); needs no blocking finding, in-target length (`requireInTarget`), the still-approved strategy, an in-allowance topic, an ACTIVE program and no open client change request; `testClientsOnly` true by default; OFF per §3 — drafting, auto-share and the share email are three separate switches; lib/scriptAutoShare.ts) · `portal_invites` (§2 Stage B) · `portal_login_email` (§2 magic links — transactional, D14) · `topic_refresh` (§18 — since CP-07 the unattended topic bank: the initial bank queued at strategy approval, per-pillar refills below target; suggestions only) · `topic_carryover` (CP-07 — scripted-but-unfilmed topics carry into the new month's allowance with their script and history; swappable by the client) · `strategy_generation` (§21) · `legacy_call_sweeps` (A04, unified handoff Sep 25 2026 — the pre-mapping sweeps in contentCalls.ts: Calendly by invitee email, Drive notes filed by the NAME in the title with a first-name fallback, Notetaker recaps. Retired: they run only with this ON and no Calendly mapping enabled; with it off they report the "map the dedicated event types" exception. Leave it off) · `session_booking` (§4 true self-booking — CP-04: books, reads back, cancels and moves in Aryeo ONLY for clients in its config `authorizedFixtureClientIds`, which must also be TEST clients; everyone else stays desk-assisted) · `address_sync` (CP-05 — PATCH a session's Aryeo address and read it back; same `authorizedFixtureClientIds` rule; with it off a client's exact address still saves and goes to Kyle, and our own order sync's readback is what confirms it) · `cut_transcripts` (§9) · `caption_assistant` (§10) · `fact_extraction` (§23 auto-accept rules) · `topic_folders` (CP-09 batch C — one raw folder per topic under the job's 02-RAW-Video, named `NN <title> [<id8>]`; adopted by bracket or Dropbox id, never renamed, moved or deleted; internal).

`portal_layout_v2` (UI-01, Sep 24 2026; reaches clients, sends nothing) — the portal's new navigation (Home · My Plan · Content Library · Schedule · More, `lib/portalLayout.ts`) for REAL clients. No `configJson`. TEST clients always get it and staff can preview any client with `?layout=v2`; with the row missing every real client keeps today's layout, and old `?tab=` links resolve in both (`lib/portalNav.ts`).
...
**Signups, batch 2 (unified handoff §6.1, Sep 25 2026). No schema change.** Package and terms come from the PRICE first (`programTermsFor`: `STRIPE_PROGRAM_PRICES` → a known product on an unknown price → the product name); anything but a known price parks the signup NEEDS_REVIEW with an "Unrecognised price …" note, and a payment below list is an informational note that does not park. `sweepStripeSignups` returns `truncated` when page 5 still has more. Find-or-create client + enrollment run in one transaction under `lockAdvisory("signup-client:<email or name>")`, so two checkouts from one new buyer land on one client; the second signup says "existing enrollment" and parks. Payer ≠ invitee with discovery booked first: `candidateDiscoveryBookingsFor(clientId)` (contentCallRecords, read-only) lists UNMATCHED discovery bookings the sync named, whose address is a proposed alias, or whose full name now matches; activation proposes the alias (never verifies it), Kyle's task on the same `program-discovery-booking:` key asks him to confirm the booking, and the welcome's step 1 is conditional. Webhook readiness: Connection `stripe_webhook` is written by the /connections "Stripe webhook" card (`connectApiKey`, `whsec_` shape check, value never echoed) or by `scripts/_ops/register-stripe-webhook.ts --apply` (the one live-Stripe write; dry run by default; `--rollback we_…`). AppSetting `stripe-webhook-coverage` = `{missed, lastMissedAt, lastCheckoutId}`, incremented with one owner bell (`stripe-webhook-missed:<checkoutId>`) when the poll activates a checkout >15 min old, opened after the secret was saved, that no webhook row names; silent with no secret. Drill: `scripts/_drill/b2-signups.ts` (port 5624) and cp14 §14–16.

## §8.1 — one responsible reviewer per cut (unified handoff, batch 1, Sep 25 2026)

**Columns and table (pushed in 2528374, nothing else).** `ReviewSubmission.reviewerTeamMemberId / reviewerAssignedAt / reviewerAssignedBy / reviewerRole` (`PRIMARY | BACKUP | FALLBACK | MANUAL | COVER`, `@@index([reviewerTeamMemberId, status])`) and `CutReviewerEvent` (one row per change: `fromTeamMemberId → toTeamMemberId`, `reason SUBMITTED | CLAIM | COVER | MANUAL | AWAY_TRANSFER | MOVED | AUTO_TRANSFER`, actor, note, `at`). No backfill migration.

**The chain (`src/lib/reviewerAssignment.ts`).** AppSetting `review_room`: `creativeApproverTeamMemberId` is now the PRIMARY seat (James), plus `backupReviewerTeamMemberId` (Kyle), `fallbackReviewerTeamMemberId` (Jordan), `coverOfferHours` (default `DEFAULT_COVER_OFFER_HOURS` = 9) and `coverTransferHours` (default `null` = never — Jordan's open §13 question). A cut is assigned ONCE, inside `announceCutInReview` (the one moment every door into the Room shares; a cut held for the editor's §8.2 check is not announced and gets no reviewer), to the first seat with an active login that is not away. Every change is a compare-and-set on the reviewer the caller read, committed with its `CutReviewerEvent` in one transaction: a race leaves one owner and one event. Nothing configured, or nobody present, = no reviewer and the old OWNER+ADMIN broadcast, byte for byte. The creative-manager flag is never read to assign or to authorise.

**Away.** AppSetting `review-away:<teamMemberId>` = `{until, setBy, setAt}` — one key per person, NOT inside `review_room`, so a stale Save of the Settings card cannot undo it. Marking a seat away moves that person's PENDING cuts to the next present seat (`AWAY_TRANSFER`); coming back moves nothing back. This is the only automatic move.

**Authority.** `requireCutReviewer` (auth/guards.ts) → `canRuleOnCuts`: OWNER/ADMIN exactly as before, or one of the three seats with an active login; never "view as"; never `TeamMember.creativeManager`. Used by `approveCut`, `requestCutChanges`, `setCutNoteStatus` (reopen/resolve), `addCutNote` (office lane), `requireCutNoteAccess`, and the new `claimCutReview`, `reassignCutReviewer`, `setCutReviewerAway` (a seat may mark only itself), `loadCutReviewerSeats`. Take-back/move, Topaz and delivery actions stay OWNER/ADMIN. A verdict by someone other than the assignee is ONE call; after the verdict's own compare-and-set wins, `recordRulingReviewer` writes a `COVER` event — never a second approval.

**Notices.** With a reviewer: the assignee gets a person row ("Waiting on you", kind `cut_ready` → their own `review_ready` switch — James's is bell-only by an earlier consent decision and is never flipped in code); Jordan gets an OWNER row with the oversight sentence ("· James has it"), his own-upload carve-out kept; every other present seat and the office (Kyle) get an FYI person row on their own switch (Kyle's Sep 21 Slack DM kept). The ADMIN role broadcast is dropped so the assignee has one bell. Take/hand-on/away/auto: `cut_ready` person row to the new owner (dedupe `cut-reviewer-<sub>-<event>`), bell-only `review_reassigned` FYI to the previous one; nobody is told about their own press. A bell row never changes a cut's status or reviewer.

**Hourly (`reviewCoverSweep`, from `syncProjectStatuses` when not single-project).** Re-runs the away move; gives an ANNOUNCED, unassigned PENDING cut on a live job the present reviewer silently (pre-deploy cuts, or cuts that arrived while all three were away); offers the backup a cut the primary has held ≥ `coverOfferHours` COVERED hours (`coverage.coveredHoursBetween`, Mon–Fri 9–6 ET by the rota) — one bell-only `review_cover_offer` per cut, nothing moves; and only when `coverTransferHours` is set, moves such a cut to the next seat once (`AUTO_TRANSFER`, never a `MANUAL` pick).

**Exceptions board.** "Waiting on a verdict" is now two COVERED days (`EXCEPTION_RULES.reviewAgingCoveredDays = 2`; was 3 calendar days), measured from `selfCheckedAt ?? createdAt`, held cuts excluded, owner = the row's own reviewer (the board-wide approver label only for a cut nobody holds). A designated seat counts as able to approve with any active login.

**Surfaces.** `/edit/<id>`: `ReviewerStrip` (who each waiting cut is with, covered hours, "I'll take it" / "I'll cover it" / "Hand to…" for the desk; the name only for an editor) and the tracker line "Ready for review — with <name>". Settings → Review Room: three seats, per-seat "no login" and "bell-only" notes, "Away until", the offer hours and the automatic-move switch (off).

**Switches.** `review_room.coverTransferHours` (null = off). No client-facing effect.

Drill: `scripts/_drill/b1-review-ownership.ts` (port 5602; clock pinned to Tue Sep 22 2026 10:00 ET; old behaviour loaded from 75d56f1).

## §7.1 — what each editor is actually working on (unified handoff, batch 1, Sep 25 2026)

**Tables (pushed in 2528374, nothing else).** `EditorWorkItem` — one row per (editorKey, projectId), `state ACTIVE | PAUSED | CLOSED`, `activeFor` = the editorKey while ACTIVE else null and `@unique`: one active project per editor is a database guarantee (a nullable unique column, not a partial index, because `db push` manages the schema). `firstStartedAt` / `activeSince` / `pausedAt` / `closedAt` + `closeReason`. `EditorWorkEvent` — the declared log (`START | RESUME | PAUSE | AUTO_PAUSE | SUBMIT | CLOSE | CONFIRM | CONFIRM_PAUSED`), actor id/name/role, `onBehalf`, `switchedTo`, `requestId @unique` (a retried click replays). No backfill; no Project column changed.

**Two axes (`src/lib/editorWork.ts`).** `Project.status` stays the lifecycle (EDITING = "editing has begun"), so payroll, the sweep and Ops counts are untouched. Who is on a job NOW is the work layer. Only `startEditing` and `confirmCurrentWork` write ACTIVE, and both need a person: the assigned editor (their assigned `editorKey`, never a login-name slug) for their own work, or OWNER/ADMIN as a labelled correction (`onBehalf`, the office's name on the event, the timeline and the bell). "View as", photographers and vendor keys (external agency, Luma — no login) are refused. Authorised = the live edit card's assignee, an open video-lane revision's assignee, or the owner of a live `DeliverableOutput` (two editors can share a job; Kim starting never pauses John).

**The switch.** One transaction behind `dbLocks.lockAdvisory("editor-desk:<key>")`: pause (or, for work that has left their hands, close) the editor's ACTIVE item, activate the target, log both, SHOT/Waiting → EDITING (REVIEW/REVISION keep their stage), card OPEN → IN_PROGRESS + `assignedManually` (so mintEditTask's re-route can never move started work). Any failure — including a racing tab's P2002 on `activeFor` — rolls the whole thing back and answers "refresh". `pauseEditing` writes only the item, one event and one timeline row. The edit_started bell rings on START, not RESUME.

**Closing (never starting).** `closeActiveWork` / `closeGhostWork`, never throwing: SUBMITTED (portal upload `uploadCutEntered`, the button `submitCutForReview` — the submitter's item only; folder discovery is not a submit), REASSIGNED/UNASSIGNED (`setEditVideoEditor`; other reassign paths are caught by `closeGhostWork` in `mintEditTask`'s refresh and left out of every reader meanwhile), REMOVED (`removeFromEditorQueue`; a restore brings the card back, not the work), PUT_BACK (the pill's Ready for editing / Waiting, the override's), PROJECT_DELIVERED/CANCELLED (`closeObsoleteTasks`, which every delivery/cancel path shares), MERGED (the donor in `mergeProjectWork`). `correctedCutWithdrawn` now writes SHOT, not EDITING.

**Readers.** `workStateFor` (batched) feeds the queue rows (`workLabel`: "In editing" only with someone ACTIVE, "Paused", "In editing — not confirmed" for an EDITING nobody confirmed, a chip on review/revision rows; the batch fallback reads Ready for editing and "N more to edit"), `videoStatesFor` (Ops Day / QC card), the edit page's `WorkStateBar` + tracker, and `workingNow()` for the office's Working-now panel (AutoRefresh 60 s; "as of", stale past 3 minutes, a failed read says so). The workload lane is "Owed to the editor".

**Historical EDITING rows.** Derived, never stored: EDITING + no work row + the live card's assignee is a team editor = "claimed — not confirmed", dated by the old "<name> started editing." line when there is one. The editor's `/editing` asks once, "Which one are you on right now?" → the pick ACTIVE (CONFIRM), the rest PAUSED with `firstStartedAt` null (CONFIRM_PAUSED); "None of them" pauses all. The live count per editor was not measured (this batch runs nothing against production); the read-only probe is `project.groupBy status=EDITING × open edit_video card assignedKey`.

**Switches.** None — internal staff behaviour, no client or provider effect.

Drill: `scripts/_drill/b1-active-editing.ts` (port 5601; clock pinned to Tue Sep 22 2026 10:00 ET; old behaviour loaded from 75d56f1). Real interleaving on regular Postgres is left to the R04 harness.

## O01 / O02 — receipt is not readiness, and an unchecked 1080p file is held (unified handoff, batch 1, Sep 25 2026)

**Raws notices (`src/lib/tasks.ts`, no schema).** Two timeline markers, each once per Waiting hold (`holdBoundary`), each claimed under a per-project advisory lock so two passes at once write one. The RECEIPT keeps the old prefix `Raws in for <street>` and now reads `… — received: <photos only | photos in, video not found yet | waiting — <the card's own blocker> | files received; readiness not checked yet | ready for editing>`; it posts ops Slack and addresses no editor. The READY marker `Ready for editing: <street>` is written by `notifyReadyToEdit` the first pass the handoff is complete with the footage in (bell dedupe `ready-<projectId>[-h<holdMs>]`, the only notice that reaches `editor:<key>`), or together with the receipt when the job is ready at receipt. A receipt written before this change (no `— received:`) counts as both, so nothing already announced is re-announced. `ensureEditorHandoff` evaluates `handoffReadiness` first and passes it to the receipt; the upload-portal fallback and the Dropbox status sweep call `notifyRawsLanded` without it and get "files received". Not changed here: the Editing Room's "Waiting on instructions" wording (editorQueue.ts, another lane).

**TopazJob columns (pushed in 2528374, nothing else).** `sourceHasAudio / sourceAudioCodec` are measured once in stepQueued (a failed probe leaves them null). `outputCheck`: `verified` (read and right), `unverified` (Topaz's link unreadable after the retries → saved to `05-Final-Video/unverified/<stem> (Topaz - unchecked).<ext>` on `heldPath`, never on finalPath/savedAt), `resolved-processed` (a person listened and chose it, attestation `HELD_ATTESTATION` in src/lib/topazHold.ts), `resolved-original` (a person kept the approved original); null = a row from before, which files exactly as it always did. `outputCheckJson` records `{verdict ok|lost|mismatch|unreadable, reason, where topaz|dropbox, source facts, output facts}`. A readable-and-wrong file (silent, running time off by more than max(1 s, 2%), short side off by more than max(8 px, 2%)) fails at either read with the original untouched. An unverified copy is read again from Dropbox: right → promoted to `FINAL (Topaz)` and the usual tail runs once; still unreadable → state `held`, `heldAt`, Topaz's copy deleted, one bell-only `topaz_problem` (dedupe `topaz-held-<jobId>`).

**State `held`.** Not live (the driver never claims it — no re-render, no re-charge) and not terminal (readyToSend lists it under rendering with the file to listen to; markVideoSent and the Aryeo proof pass treat it as the lane still owing work; `retryTopazJob` refuses it). It ends only by `resolveHeldTopazJob` (`use-original` → `failed`, file moved to superseded/, editor export renamed FINAL (editor export); `accept-processed` → `done`) or `recheckHeldTopazJob` (Dropbox only) — each a compare-and-set claim on `resolvedAt` with `resolvedBy/resolution/resolutionNote`. Server actions `resolveHeldRenderAction` / `recheckHeldRenderAction` (review/actions.ts) use `requireCutReviewer`. Exceptions board kind `unverified-render`. The hub's blob copy of a held cut is not pruned.

**Switches.** None: internal notices only, no client or provider write.

Drill: `scripts/_drill/b1-readiness-topaz.ts` (port 5604; clock pinned to Wed Sep 23 2026 10:00 ET; old behaviour loaded from 75d56f1; Topaz and Dropbox scripted at the module boundary, Slack held at fetch).

## §8.2–8.4 — editor self-check, revision issues, editor quality (unified handoff, batch 1, Sep 25 2026)

**Tables (pushed in 2528374, nothing else).** `CutSelfCheck` (the attestation: `checklistKey` `<style>@v<n>`, `itemsJson`, `addressedIssueIdsJson` = `{addressed, notAddressed}`, `attestedFileName/Size`, `fileIdentity`, `sourceRev`, `editorKey` = the editor it is FOR, `actorName/actorUserId` = who ticked, `onBehalfOf` when the office attests, `state PENDING_BYTES | VALID | VOID`). `ReviewSubmission.selfCheckId / selfCheckedAt / sourceRev`. `RevisionIssue` / `RevisionIssueEvent`. No backfill.

**Held vs in review (`src/lib/selfCheck.ts`).** A cut is HELD when it is PENDING, carries a check id, and has no `selfCheckedAt`. Grandfathering is by that MARKER, not by clock: a pre-gate row (or one a drill builds by hand) has no check id and is never held. `awaitingReviewWhere()` / `awaitingSelfCheckWhere()` / `isHeldForSelfCheck()` are the one test; opsExceptions, reviewerAssignment and videoStatesFor use it. `SELF_CHECK_REQUIRED_SINCE` (Sep 25) only dates `imported` issues and the quality report's start.

**Every door (`src/lib/selfCheckStore.ts`, `reviewCuts.enterReview / claimReviewEntry`).** Upload: `startCutUpload` refuses without a complete check (nothing reserved) and writes it `PENDING_BYTES` in the reservation's transaction; `finalizeCutUpload` binds it when the landed size and reserved name match (`VALID`, `fileIdentity = stableCutIdentity`) — a mismatch VOIDs it and HOLDS the cut (bell-only `self_check_needed` to the editor and the office; the browser does not abandon the bytes); the store's size-less callback binds on the next call that has a size. Final folder: `submitCutForReview` without a check writes nothing and returns `needsSelfCheck` + the candidate file; with one it mints/claims the row (held), binds it to Dropbox's `content_hash` read at that moment, then ONE compare-and-set on `selfCheckedAt` (`claimReviewEntry`) decides who runs the side effects. Sweep: rows are created held, carry `sourceRev`, and are NOT announced. Move: a gated cut lands held on the target with its check VOID; its entry (`movedCutEntered`) announces with the move's dedupe key. Restore keeps the prior round's own check. `approveCut` / `requestCutChanges` refuse held cuts and re-read a folder cut's hash (`folderDriftCheck`: changed → VOID + held, unreadable → refused with a retry). `submitSelfCheck` (review/selfCheckActions) finishes a held cut — the editor for their own, the office on their behalf. The queue pill relays "open the edit page — the check comes first".

**Checklist.** `DEFAULT_SELF_CHECK` per Style Guide key (§8.2's seven lines; captions/brand never N/A where the product carries them); refinements in AppSetting `editor_self_check` via `saveSelfCheckProfile` (review desk), saved as a NEW version; "watched the actual export" cannot be removed. N/A and "not done" need a reason (≥ `SELF_CHECK_REASON_MIN` = 8 chars).

**Issues (`src/lib/revisionIssues.ts`).** Ingested idempotently on `(sourceKind, sourceId)`: EDITOR-lane root fix notes by the desk or the shooter (`addCutNote`, `askCutChange`, and any still-open note at `requestCutChanges`), and video-lane `RevisionBrief` items per item × named video (`${brief}:${item}:${slotKey|job}`; an unanalysed brief is one job-level issue; a brief analysed to nothing makes none). Portal comments come in only through their pinned brief. Re-analysis reconciles (vanished → NOT_APPLICABLE + `REANALYSED`) and is REFUSED once any of the brief's issues is classified, addressed, verified or merged. The editor's "done" is their check (`ADDRESSED`, bound to the version); the legacy note toggle and work-order ticks are mirrored (FIXED → ADDRESSED, RESOLVED → VERIFIED; ticks on read). `approveCut(id, {verifyIssueIds})` verifies in that version; with the list, an unticked ADDRESSED issue refuses; on a checked version an earlier issue the editor declared not done refuses until marked not needed; the plain button verifies the ADDRESSED ones. `requestCutChanges(id, {notFixedIssueIds})` reopens named fixes (`missedInSubmissionId`) and stamps earlier undeclared asks missed on a checked version. `assignedEditorKey` follows the edit card (lazy, on read); `versionEditorKey` (the row's key, else the accepted check's editor) is never rewritten. Causes: `EDITOR_ERROR | MISSED_INSTRUCTION` (editor-caused) `| CLIENT_CHANGE | NEW_SCOPE | BRIEF_GAP | CAPTURE | PROCESSING | UNCLASSIFIED`; only `classifyIssue` (review desk) writes `cause`; `causeSuggested` is the AI's slot. Merge (duplicate → root; a closed root reopens when asked again) and split are events. ContentRevisionRound, fees, payroll, bonus, kpi.ts and QcRecord are untouched.

**Quality (`src/lib/editorQuality.ts`, /quality?tab=editors).** Live from the ledger, from Sep 25: first-review pass = round-1 versions with no confirmed editor-caused issue ÷ round-1 versions with a verdict (pending, unclassified and replaced-before-review reported beside it, never in the denominator); recurring = distinct editor-caused roots by category × product with examples and weekly counts; missed corrections; revision turnaround in elapsed WEEKDAY hours (§3's meaning) with the 1080p pass, waiting-on-assets (§7.1 pauses with a reason, else `null` = "not recorded") and the client's total wait beside it; found-after-approval split editing / review miss / unclassified; review waiting per reviewer queue from `selfCheckedAt ?? createdAt`, split by `CutReviewerEvent`. Every block carries n; below `THIN_SAMPLE` = 5 it says "not enough to judge". Owner/admin and review seats see one card per editor (roster order, no ranking) plus "Issues waiting on a cause" and the checklist editor; an EDITOR sees only their own card.

**Surfaces.** `/edit/<id>`: the upload panel's check dialog (`SelfCheckDialog`), "Waiting on the send-for-review check" (`HeldCutsCard`, anchor `#self-check`), the folder button (`FolderSendForReview`), and "Revision issues" (`RevisionIssuesPanel`, anchor `#issues`; reviewers classify / verify / merge / split / mark not needed and read each version's check). Not mounted in the Review Room's own panel (CutReviewPanel belongs to the §8.1 surface) — its Approve/Send-back use the plain calls above.

**Switches.** None new. Folder discovery stays behind `review_room.discoverFromDropbox` (off).

Drill: `scripts/_drill/b1-selfqc-issues.ts` (port 5603; old behaviour loaded from 75d56f1; §8.4 times on a pinned Friday→Monday; Dropbox, the model and the signed-in user faked at the module boundary).

## R01 / §6.4 / §11 — one planning reader, the two guided routes, Your Month (unified handoff, batch 2, Sep 25 2026)

**Columns (pushed in 2528374, nothing else).** `ContentMonth.planningChosenAt / planningChosenBy` — stamped by `setPlanningMode(monthId, mode, by)` (the only writer of `planningMode`), so an explicit route is told apart from the call mode's default; `by` is `client:<id>`, `staff:<id>` or `link`. `ContentMonth.schedulingDeferredAt / schedulingDeferredBy` — "Schedule later", stamped once by `portalScheduleLater` (a second tap keeps the first stamp), kept as history after a booking. No backfill.

**The reader (`src/lib/planningState.ts` pure, `src/lib/planningFacts.ts` server).** `allowanceOrder` decides which live selections (`ALLOWANCE_SELECTION_STATUSES`, plus a topic pointing at the month with no live selection, read as SELECTED) are the month's allowance: CARRIED first, then SELECTED/RECONCILED by rank then createdAt, PROPOSED last; the first `videosOwed` are IN, the rest EXTRA. The stored `ContentTopicSelection.overflow` flag is no longer read by any planning surface. `topicPlanStep` gives each topic one step (`FILMED | EXTRA | APPROVED | CHANGES_REQUESTED | READY_FOR_YOU | TEAM_REVIEW | CONFIRMING | WRITING | NEEDS_MORE | ON_CALL | CHOSEN | NEEDS_ANSWERS`); `monthPlanning` gives chosen, extras, per-step counts, `answersOwed` (IN topics at NEEDS_ANSWERS/NEEDS_MORE), `approved`, a client headline and "N of M scripts approved". Scripts read are this month's hub scripts plus historical imports filed on THIS month (another month's import never hides a gap); the client's decision is `scriptDecisions.currentScriptDecision`, "released" is `postingKit.scriptVisibility`. Call material = client/Jordan lines (> 20 chars) from the selection's evidence, else the newest DISCUSSED event from this month's call (the KEPT branch), after `clientFacts.confidentialFilter` (clients with no confidential fact: the two markers, `markedConfidential`) and `contentGeneration.otherClientScrubbers`. Route and call come from `deriveMonthState` over the month's records. Every read runs whatever the months hold, so a roster costs the same statements as one month.

**Consumers.** `portalTopics` (`PortalTopic.plan`, `PortalTopicMonth.planning`, `selection.overflow` derived, the pillar-less client group "Your ideas"/"More ideas"), `planModel.toAnswer`, `homeActions` (ANSWER_QUESTIONS on what is owed, new CHOOSE_ROUTE, links into `#step-<key>`), `portalPlanning` (`interviewsOpen`/`answersSubmitted` over the allowance, effective call mode, `chosenAtISO`, `deferredAtISO`, `planning`), `monthProgressMany` (`topics.selected` = IN, `topics.overflow` = EXTRA — the extra is no longer counted twice — and `planning`; the staff next step names "proposed on the call — keep or drop"), `recalcProgramMonth` (only IN topics are required material; an extra's script does not hold the month), `scriptWorkForMonth` (readiness `EXTRA`, never auto-drafted; call material from the reader), `sweepInterviewPlans` (skips EXTRA / ON_CALL / WRITING / TEAM_REVIEW).

**Route rules (`programMonths`).** `effectiveCallMode`: an explicit `callMode` wins; the legacy-derived REQUIRED (column null, flag true) holds until a monthly strategy call was held in ANOTHER month (`priorCallHeldFrom`: MATCHED/CONFIRMED_BY_STAFF and held, or a month stamped COMPLETED), then OPTIONAL_WRITTEN unless `noCallEligible === false`. `deriveMonthState` uses it, so the reminder evaluator's BOOK_CALL/CHOOSE_PATH and the portal's offer follow without their own reading; an UNDECIDED month with scripts is prepared (scripts are proof planning happened). `earliestFilmingStart` is the one earliest-start function (W01 changes the window there). `sessionGate` treats a booked call as booked until its END (it read the start, so the calendar locked during the call). Written route + a live call: new offers start no earlier than the call's end + the window, and an unfilmed booked session inside that buffer is an exception + a `CALL_INSIDE_BUFFER` follow-up; a non-dry `recalcProgramMonth` raises/clears one Kyle desk task `program-call-buffer:<monthId>` (never for TEST; the booking is never touched).

**Portal (v2, `portal_layout_v2`; TEST clients always).** Nav "Your Month" (key `plan`). The month view is `YourMonth` over `lib/yourMonth.yourMonthSteps`: route → topics → answers | call → filming (the Schedule page's `PortalScheduler`, `embedded`, plus "Schedule later") → scripts; at most one current step. Schedule keeps appointments and links its route question to `#step-route`. Plain labels in `portalWords.CTA_WORDS`; per-topic chips in `PLAN_STEP_WORDS` / `planStepWord`. v1 keeps its layout; its counts and chips read the same reader.

**Switches.** None new. Reminders (off) read the derivation; drafting/question sweeps stay behind `script_drafting` + `ai_runs` (off).

Drill: `scripts/_drill/b2-planning.ts` (port 5621; clock pinned to Wed Oct 7 2026 10:00 ET; old behaviour loaded from f2555f7).
