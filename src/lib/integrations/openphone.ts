...
export const OpenPhone = {
  ...
  messages: (phoneNumberId: string, participants: string[], maxResults = 20) =>
    openphoneRequest<Paged<OpMessage>>("/messages", { query: { phoneNumberId, participants, maxResults } }).then(
      (r) => r.data ?? [],
    ),
  calls: (phoneNumberId: string, participants: string[], maxResults = 20) =>
    openphoneRequest<Paged<OpCall>>("/calls", { query: { phoneNumberId, participants, maxResults } }).then(
      (r) => r.data ?? [],
    ),
};
