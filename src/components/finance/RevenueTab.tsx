...
// Revenue tab = the old Sales Tracker, unchanged. Owner-only (Finance's money
// overview). Runs its own project query — the Unpaid + Payroll tabs never do.
function StatCard({
  icon: Icon,
  label,
  value,
...
export async function RevenueTab({ show }: { show: FinanceTab[] }) {
  ...
  // Outstanding balances — SAME definition as the Unpaid tab (delivered + still
  // owing, non-cancelled) so the two tabs can never show contradictory money.
  ...
  return (
    <div>
      ...
      <div className="space-y-6 p-6">
        <FinanceTabs tab="revenue" show={show} />
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-5">
          ...
          <StatCard
            ...
            accent="#dc2626"
            sub={`${outstandingOrders.length} delivered & unpaid — matches Unpaid tab${preDeliveryTotal > 0 ? ` · +${formatMoney(preDeliveryTotal)} invoiced, not yet delivered` : ""}`}
          />
