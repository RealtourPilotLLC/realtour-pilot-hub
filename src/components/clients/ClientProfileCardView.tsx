...
import { UserRound, RefreshCw, Sparkles, ThumbsUp, ThumbsDown, MessageSquare, Repeat, Camera, Palette, ShieldCheck, Scissors } from "lucide-react";
import { regenerateClientProfile } from "@/app/clients/actions";
import type { ClientProfile, ProfileFacts, RevisionAsk } from "@/lib/clientProfile";
import { ink } from "@/components/ui/Badge";
import { Bullets, RecentAsks, Stat } from "@/components/clients/profileParts";

// THE FULL working profile — owner and admin only (see ClientProfileCard, which
// picks the audience). Editors get EditorProfileCard instead: Jordan, Sep 2
// 2026, on what Kim and John were being shown, "there is too much there".
...
export function ClientProfileCardView({
  clientId,
  profile,
  facts,
  asks,
  ...
  /** Live counts — the stored profile's own numbers are as old as the build. */
  facts: ProfileFacts;
  asks: RevisionAsk[];
        ...
        {!profile ? (
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <Stat value={facts.inboundMsgs} label="Messages" />
            </div>
            ...
            {/* The editor's own block, shown here so Jordan and Kyle can see
                exactly what Kim and John are reading on /edit. */}
            <Bullets icon={<Scissors className="size-3.5" />} title="Editing preferences" items={profile.editing?.prefs ?? []} />
            <Bullets icon={<UserRound className="size-3.5" />} title="Customer notes (editor)" items={profile.editing?.customerNotes ?? []} />
